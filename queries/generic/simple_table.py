...
def two_line_table(con, a1, a2):
    df = pandas.read_sql_query(
        "SELECT Actual_Date_"+ a1 +" AS DATE, Actual_"+ a1 +", Actual_"+ a2 +" FROM "+ a1 +" LEFT JOIN "+ a2 +" ON Actual_Date_"+ a1 +" = Actual_Date_"+ a2 +" LIMIT "+ NUM_ROWS, con)
    return df
...
def simple_table_to_plot(con, out, num_rows, name, a, *args):   #At least one index in inputs
    global NUM_ROWS
...
if __name__ == "__main__":
    ...
    num = 26
    name = "Total_Reserves_of_Depository_Institutions_vs_Reserves_Balances_Required"
    simple_table_to_plot(con, out, num, name, "FTRDIM", "FRBRM")
